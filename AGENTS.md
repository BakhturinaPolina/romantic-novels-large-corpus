...
- Pattern new BO selection analysis notebooks after `notebooks/04_selection/`, driven by YAML configs such as `configs/selection_notebooks.yaml`.
...
- BO config: 130 calls, 3 `model_runs` per call, topic-count-penalized coherence objective, topic stability enabled (`configs/train_v3*.yaml`).
- `transfer_bundle/` is built via `scripts/make_transfer_bundle.sh`; remote runs use `scripts/run_v3_remote_model.sh`; CPU encode on remote, GPU required for cuML BO tuning.
