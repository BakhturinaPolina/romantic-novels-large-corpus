...
#
# Usage:
#   ./scripts/run_v3_embed_local.sh minilm6 encode
#   ./scripts/run_v3_embed_local.sh minilm6 tune          # start or resume BO
#   ./scripts/run_v3_embed_local.sh minilm6 tune-status
#   ./scripts/run_v3_embed_local.sh minilm6 tune-logs
#   ./scripts/run_v3_embed_local.sh minilm6 eta-watch
set -euo pipefail
...
cmd_encode() {
  ...
  if [[ -f "$ROOT/$CACHE_FILE" ]] && [[ ! -f "$ROOT/${CACHE_FILE}.progress.json" ]]; then
    ...
    "$ROOT/scripts/embed_v3_eta.sh" "$model"
    exit 0
  fi
  ...
  echo "ETA: $ROOT/scripts/embed_v3_eta.sh $model --watch"
...
PY
  ...
  sleep 3
  "$ROOT/scripts/embed_v3_eta.sh" "$model" || true
}

cmd_eta() {
  "$ROOT/scripts/embed_v3_eta.sh" "$1"
}

cmd_eta_watch() {
  "$ROOT/scripts/embed_v3_eta.sh" "$1" --watch
}
