...
#   B) Stage05b test holdout (chunked)
#   C) full-corpus sentence-level inference (train + val + test)
#
# Usage:
#   nohup ./scripts/run_v4_call73_full_corpus.sh >> logs/v4_call73_full_corpus_console.log 2>&1 &
#
...
BO_CALL="${BO_CALL:-73}"
PATHS_CONFIG="${PATHS_CONFIG:-configs/paths_stage03_fit_v3.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/train_v4_l12_final_call73.yaml}"
...
# --- Step B: Stage05b test holdout (chunked) ----------------------------------
if [[ "$FORCE" == "1" ]] || [[ ! -f "$HOLDOUT_METRICS" ]]; then
  log_eta "Step B Stage05b test holdout (~17.5M sentences, chunked)" "3-8"
  step_start=$(date +%s)
  echo "[$(ts)] > Step B | holdout scoring on test split"
  "$PY" -m src.stage05b_test_holdout.cli score \
...
if [[ "$FORCE" == "1" ]] || [[ ! -f "$INFER_SUMMARY" ]]; then
  log_eta "Step C full-corpus infer (train+val+test ~115M sentences)" "24-48"
  step_start=$(date +%s)
  echo "[$(ts)] > Step C | chunked transform train,val,test -> parquet"
  "$PY" -m src.stage05_final_fit.cli infer-corpus \
    --model-dir "$MODEL_DIR" \
    --run-id "$RUN_ID" \
    --paths-config "$PATHS_CONFIG" \
    --config "$TRAIN_CONFIG" \
