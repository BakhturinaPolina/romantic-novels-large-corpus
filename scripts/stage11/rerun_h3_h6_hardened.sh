#!/usr/bin/env bash
# Re-run H3→H4→H5→H6 with hardened v1.1 prompts (--no-resume each).
# H5/H6 use 06_run_h5_h6_audits.py; H3/H4 use 05_run_hypothesis_audits.py.
#
# Usage:
#   bash scripts/stage11/rerun_h3_h6_hardened.sh
#   WAIT=1 bash scripts/stage11/rerun_h3_h6_hardened.sh   # wait for other workers first
#   ONLY=H5,H6 bash scripts/stage11/rerun_h3_h6_hardened.sh
#   MODEL=anthropic/claude-sonnet-4.6 bash scripts/stage11/rerun_h3_h6_hardened.sh
...
WAIT="${WAIT:-0}"
ONLY="${ONLY:-H3,H4,H5,H6}"
PY="${PY:-.venv/bin/python}"
LOG_DIR="$ROOT/logs"
PROGRESS="$LOG_DIR/h3_h6_hardened_rerun_progress.txt"
mkdir -p "$LOG_DIR"
...
run_audit_05() {
  ...
  note "START AUDIT $hyp (05) model=$MODEL --no-resume log=$log"
...
run_audit_06() {
  local hyp="$1"
  local log="$LOG_DIR/${hyp,,}_sonnet_hardened_rerun.log"
  note "START AUDIT $hyp (06_run_h5_h6) model=$MODEL --no-resume log=$log"
...
note "H3–H6 hardened re-run start MODEL=$MODEL WAIT=$WAIT ONLY=$ONLY"

if [[ "$WAIT" == "1" ]]; then
  note "Waiting for other audit workers to finish..."
...
note "H3–H6 hardened re-run ALL DONE"
