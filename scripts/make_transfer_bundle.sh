...
for cfg in "$BUNDLE/configs/train_v3_mpnet.yaml" "$BUNDLE/configs/train_v3_minilm6.yaml"; do
  if [[ -f "$cfg" ]]; then
...
# Remote-run helper script (also copied into bundle scripts/).
mkdir -p "$BUNDLE/scripts"
cp "$ROOT/scripts/run_v3_remote_model.sh" "$BUNDLE/scripts/"
chmod +x "$BUNDLE/scripts/run_v3_remote_model.sh"
...
Mirrors the repo `data/` layout expected by `configs/paths_stage03_fit_v3.yaml`
and `configs/train_v3_*.yaml`. Small files are pre-filled by
`scripts/make_transfer_bundle.sh`; copy the large files listed below before
starting Docker.
...
| Path | Size (approx.) | Notes |
|------|----------------|-------|
| `interim/octis/v3_english_only/embeddings_cache/train_eval_sentence-transformers__paraphrase-mpnet-base-v2.npy` | ~280 GB | MPNet CPU encode (resumable; slow) |
| `interim/octis/v3_english_only/embeddings_cache/train_eval_sentence-transformers__paraphrase-MiniLM-L6-v2.npy` | ~140 GB | MiniLM-L6 CPU encode (resumable; slow) |

Use `./scripts/run_v3_remote_model.sh mpnet encode` or `minilm6 encode` — see bundle `README.md`.
...
Referenced by `configs/paths_stage03_fit_v3.yaml`.
EOF
...
echo "  ./scripts/run_v3_remote_model.sh mpnet encode   # or minilm6"
echo
