#!/usr/bin/env bash
# Quick Stage08 status: label JSONs by subfolder, topic counts, latest log.
set -euo pipefail
...
if [[ -d "$OUT" ]]; then
  shopt -s nullglob
  mapfile -t files < <(find "$OUT" -name 'labels_pos_*.json' -type f | sort)
  if ((${#files[@]} == 0)); then
...
echo ""
echo "Progress doc: results/reports/stage08/stage08_progress.md"
