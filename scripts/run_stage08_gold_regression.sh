#!/usr/bin/env bash
# Stage 08 gold regression: label 30-topic panel and validate against call73 gold YAMLs
set -euo pipefail
...
CONFIG="configs/stage08_labeling.yaml"
TOPIC_IDS="1,2,7,14,26,40,56,66,70,72,78,84,108,118,122,123,138,140,152,161,174,210,218,248,257,277,284,292,303,326"
OUTPUT_SUFFIX="gold30_regression"
TS="$(date +%Y%m%d_%H%M%S)"
LOG="logs/stage08_gold_regression_${TS}.log"
mkdir -p logs results/stage08_llm_labeling/gold_regression

echo "=== Stage 08 gold regression ===" | tee "$LOG"
...
"$PY" -m src.stage08_llm_labeling.openrouter_experiments.tools.validate_label_quality \
  ...
  --output-csv "results/stage08_llm_labeling/gold_regression/gold30_report_${TS}.csv" \
  2>&1 | tee -a "$LOG"
