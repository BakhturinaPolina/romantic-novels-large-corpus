...
LOGGER = logging.getLogger("stage2_radway_functions")
logging.basicConfig(
    level=logging.INFO,
...
#    Reuses Stage 1 taxonomy data as additional signals.
# ---------------------------------------------------------------------------
...
- Topics whose Stage 1 taxonomy group is "Relationship Trajectory (Main Couple)"
...
- Refer to:
  ...
  - Stage 1 taxonomy group and category,
...
def load_topics_with_taxonomy_from_json(taxonomy_json_path: Path) -> Dict[int, Dict[str, Any]]:
    """
    Load Stage 1 taxonomy JSON (like taxonomy_mappings_*.json)
...
def _format_taxonomy_for_prompt(topic_entry: Dict[str, Any]) -> Dict[str, str]:
    """
    Turn Stage 1 taxonomy mapping for a topic into strings for the user prompt.
...
def map_all_topics_to_radway(
    *,
    ...
    ----------
    taxonomy_json_path:
        Path to Stage 1 taxonomy mappings JSON file OR BERTopic model with embedded taxonomy metadata.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Stage 2: Zero-shot Radway function mapping of BERTopic topics using Mistral via OpenRouter.",
